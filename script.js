...
function updateMediumCard(direction) {
  ...
  setTimeout(() => {
    ...
    updateIndicators();
  }, 800); // Ajustado para 800ms para sincronizar com a transição do CSS
}
...
// Adicionando suporte a gestos de deslizar (swipe) apenas no mobile
if (window.innerWidth <= 768) {
  ...
  mediumCardSlider.addEventListener("touchstart", function (event) {
    ...
    pauseAutoSlide(); // Pausa o auto-slide ao iniciar o toque
  });
  ...
  mediumCardSlider.addEventListener("touchend", function () {
    ...
    if (Math.abs(swipeDistance) > minSwipeDistance) {
      if (swipeDistance > 0) {
        ...
        console.log("Deslizou para a direita (anterior)");
        currentIndex = (currentIndex - 1 + covers.length) % covers.length;
        updateMediumCard("left");
      } else {
        // Deslizou para a esquerda (próximo)
        console.log("Deslizou para a esquerda (próximo)");
        currentIndex = (currentIndex + 1) % covers.length;
        updateMediumCard("right");
      }
      resetAutoSlide(); // Reinicia o auto-slide após o swipe
    }
    resumeAutoSlide(); // Retoma o auto-slide ao finalizar o toque
  });
}
